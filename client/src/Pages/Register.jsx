...
const Register = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            let { data } = await axios.post("http://localhost:5000/signup", credentials)
            if (data.success) {
                console.log(data)
                notify(data.message)
                move('/login')
            ...
            <div className='mt-12'>
                ...
                <div className='flex justify-center mt-12'>
                    <form className='flex flex-col gap-3 w-72' onSubmit={handleSubmit}>
                        ...
                        <Input color='gray' required style={{ color: "white" }} autoComplete='off' value={credentials.username} name="username" onChange={handleChange} label='Username'></Input>
